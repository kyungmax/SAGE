...
- `SAGE_DATA_DIR`: dataset HDF5 root. Defaults to `$HNSW_PLAYGROUND_ROOT/datasets` when available, otherwise `<repo>/datasets`.
- `SAGE_INDEX_DIR`: hnswlib index root. Defaults to `$HNSW_PLAYGROUND_ROOT/index` when available, otherwise `<repo>/index`.
...
- `SAGE_PYTHON`: Python executable for child cells. Defaults to `/home/kyungmin/anaconda3/envs/hnsw/bin/python3` when present.
